...
class SQLighter:
  def __init__(self):
        self.conn = MySQLdb.connect(host="localhost", user="root", password="zG1cNnxwjz", db="bot_db",charset = "utf8", use_unicode = True) #,charset='utf8'
...
db_create('nakrutka_bot',['id', 'username','date' ,'balance', 'last_service','last_amount','last_channel','last_price'])
...
def create_task_panel(chan,amount,serv):
    ...
    params = {'key': API, 'action': 'add','service':serv,'link':chan,'quantity':amount}
    z=requests.post(url,params=params)
    print(z.text)
...
def bulkfollows_order(chan,amount,service_id):
    ...
    params = {'key': API, 'action': 'add','service':service_id,'link':amount,'quantity':chan}
    print(params)
    z=requests.post(url,params=params)
    print(z.text)
